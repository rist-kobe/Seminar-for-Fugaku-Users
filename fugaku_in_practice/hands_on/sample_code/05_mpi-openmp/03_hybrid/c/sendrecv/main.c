...
int main (int argc, char **argv)
{
  ...
  MPI_Status status; 
  tag = 999;
  send_buff = (double *) malloc ( max_buffer_size*sizeof(double) );
  recv_buff = (double *) malloc ( max_buffer_size*sizeof(double) );
  ...
  fprintf(fp, "rank_src_x  =%d\n", rank_src_x);
  fprintf(fp, "rank_dest_x =%d\n", rank_dest_x);
  fprintf(fp, "rank_src_y  =%d\n", rank_src_y);
  fprintf(fp, "rank_dest_y =%d\n", rank_dest_y);
  fprintf(fp, "nxsize_local=%d\n", nxsize_local);
  fprintf(fp, "offset_x    =%d\n", offset_x);
  fprintf(fp, "nysize_local=%d\n", nysize_local);
  ...
  MPI_Sendrecv(&send_buff[0], nysize_local, MPI_DOUBLE, rank_dest_x, tag,
               &recv_buff[0], nysize_local, MPI_DOUBLE, rank_src_x , tag, 
               cartComm, &status);
  forward_dx_mpi (nxsize_local1,nxsize_local,nysize_local, &recv_buff[0], f, fdx);
  ...
  MPI_Sendrecv(&send_buff[0], nxsize_local, MPI_DOUBLE, rank_dest_y, tag,
               &recv_buff[0], nxsize_local, MPI_DOUBLE, rank_src_y , tag, 
               cartComm, &status);
  forward_dy_mpi (nysize_local1,nxsize_local,nysize_local, &recv_buff[0], f, fdy);
  ...
  for ( it = 1; it <= NITR ; ++it ) {
    ...
    /* p2p communication along x-direction */ 
    MPI_Sendrecv(&send_buff[0], max_buffer_size, MPI_DOUBLE, rank_dest_x, tag,
                 &recv_buff[0], max_buffer_size, MPI_DOUBLE, rank_src_x , tag, 
                 cartComm, &status);
    forward_dx_mpi (nxsize_local1,nxsize_local,nysize_local, &recv_buff[0], f, fdx);
    ...
    /* p2p communication along y-direction */ 
    MPI_Sendrecv(&send_buff[0], max_buffer_size, MPI_DOUBLE, rank_dest_y, tag,
                 &recv_buff[0], max_buffer_size, MPI_DOUBLE, rank_src_y , tag, 
                 cartComm, &status);
    forward_dy_mpi (nysize_local1,nxsize_local,nysize_local, &recv_buff[0], f, fdy);

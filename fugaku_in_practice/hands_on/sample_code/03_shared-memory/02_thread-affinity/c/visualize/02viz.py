...
if 'spread' in lb:
    lb = 'OMP_PROC_BIND = spread'
else:
    lb = 'OMP_PROC_BIND = close'
ax.plot(x,y,linestyle='--',linewidth='1.0',color='green',label=lb,marker='o', markerfacecolor='none')
if argc == 4:
     x2 = data2_fltp['NT']
     y2 = data2_fltp['Rate_MB/s'] * 1.0e-3
     lb2 = data2_fltp['BN'].iloc[0]
     if 'spread' in lb2:
         lb2 = 'OMP_PROC_BIND = spread'
     else:
         lb2 = 'OMP_PROC_BIND = close'
     ax.plot(x2,y2,linestyle='--',linewidth='1.0',color='magenta',label=lb2,marker='^', markerfacecolor='none')

ax.legend(fontsize='8.5',ncol=3)

ax.set_title('STREAM:Triad '+'[Fujitsu C Clang (fj4.12.1) -Ofast -ffj-zfill, FLIB_BARRIER=SOFT,\n' \
